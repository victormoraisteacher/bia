...
function pickVoices() {
  ...
  // Inglês americano, preferindo vozes femininas naturais conhecidas
  const enPreferred = voices.find(v => /en-US/.test(v.lang) && /female|woman|samantha|zira|susan|aria|jenny/i.test(v.name));
  const enAny = voices.find(v => /^en-/.test(v.lang));
  vmVoiceEN = enPreferred || enAny || null;

  // Português do Brasil, preferindo vozes femininas naturais conhecidas
  const ptPreferred = voices.find(v => /pt-BR/.test(v.lang) && /female|mulher|luciana|francisca/i.test(v.name));
  const ptAny = voices.find(v => /pt-BR/.test(v.lang)) || voices.find(v => /^pt/.test(v.lang));
  vmVoicePT = ptPreferred || ptAny || null;
}

// Reconhece blocos em inglês dentro de um texto majoritariamente em português.
// Estratégia: qualquer trecho entre aspas (retas " " ou curvas “ ”) é tratado
// como a fala do personagem em inglês; o resto é a orientação da Bia em
// português. Isso casa exatamente com o formato usado no prompt da Bia, que
// sempre escreve as falas do diálogo entre aspas e a tradução entre parênteses
// logo depois (a tradução entre parênteses é removida, pois falar os dois
// idiomas da mesma frase duas vezes seria redundante ao ouvido).
function segmentByLanguage(text) {
  // Primeiro removemos as traduções em parênteses (ex: "(Claro, aqui está)")
  // que vêm imediatamente coladas a uma frase em inglês — são para LEITURA,
  // não para fala, já que a Bia já fala em português no resto da orientação.
  const withoutParenTranslations = text.replace(/\([^)]*\)/g, "");

  const segments = [];
  // Quebra mantendo o delimitador, capturando tanto aspas retas quanto curvas
  const regex = /["“”]([^"“”]+)["“”]/g;
  let lastIndex = 0;
  let match;

  while ((match = regex.exec(withoutParenTranslations)) !== null) {
    const before = withoutParenTranslations.slice(lastIndex, match.index);
    if (before.trim()) segments.push({ lang: "pt", text: before });

    const quoted = match[1];
    // Heurística simples e rápida: se o trecho citado tem letras típicas de
    // inglês comum e poucos acentos/ç, tratamos como inglês; senão, português.
    const looksEnglish = /^[a-zA-Z0-9\s.,!?'\-]+$/.test(quoted) && /\b(the|is|are|you|i|to|how|what|please|sure|here|gate|hello|good|can|do|have)\b/i.test(quoted);
    segments.push({ lang: looksEnglish ? "en" : "pt", text: quoted });

    lastIndex = regex.lastIndex;
  }
  const rest = withoutParenTranslations.slice(lastIndex);
  if (rest.trim()) segments.push({ lang: "pt", text: rest });

  // Se nada foi capturado (texto sem aspas), trata o texto todo como português
  if (!segments.length) segments.push({ lang: "pt", text: withoutParenTranslations });
...
function cleanSegmentForSpeech(text) {
  return text
    ...
    .replace(/\n/g, " ")
    .replace(/\s{2,}/g, " ")
    // Remove rótulos curtos de personagem que sobraram isolados (ex: "Officer:",
    // "Airport Staff:") — são identificação visual, não precisam ser falados.
    .replace(/^[A-Za-zÀ-ú\s]{2,25}:\s*$/, "")
    .trim()
    // Remove pontuação órfã que sobra no início após as remoções acima
    // (ex: ". Repita em voz alta" -> "Repita em voz alta")
    .replace(/^[.,:;\s]+/, "")
    .trim();
}
...
function speakThenListen(rawText) {
  ...
  const rawSegments = segmentByLanguage(rawText);
  const segments = rawSegments
    .map(function (seg) { return { lang: seg.lang, text: cleanSegmentForSpeech(seg.text) }; })
    .filter(function (seg) { return seg.text; });

  const previewText = segments.map(function (s) { return s.text; }).join(" ");
  el("vm-transcript").textContent = previewText.slice(0, 220);

  if (!segments.length) { vmListen(); return; }

  synth.cancel();
  setVmStatus("speaking", "Falando" + "<span class='vm-status-dot'></span>");

  speakQueue(segments, 0, synth);
}

function speakQueue(segments, index, synth) {
  if (!vmActive) return;
  ...
  if (seg.lang === "en") {
    utter.lang = "en-US";
    if (vmVoiceEN) utter.voice = vmVoiceEN;
    // Inglês de prática: um pouco mais devagar, para o aluno acompanhar a pronúncia.
    utter.rate = 0.88;
    utter.pitch = 1.03;
  } else {
    utter.lang = "pt-BR";
    if (vmVoicePT) utter.voice = vmVoicePT;
    // Português de orientação: ritmo natural de conversa, não apressado.
    utter.rate = 0.95;
    utter.pitch = 1.0;
  }

  utter.onend = function () {
    speakQueue(segments, index + 1, synth);
  };
